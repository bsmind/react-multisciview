...
class TreeView extends React.Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        return !(
            nextState.expandedNodes.length === expandedNodes.length && 
            nextState.filteredNodes.length === filteredNodes.length
        );
    }
    ...
    recursiveRender = (nodes) => {
        ...
        const filteredNodes = this.state.filteredNodes.length > 0 
            ? nodes.filter(key => this.state.filteredNodes.indexOf(key) > -1)
            : nodes;
